...
class FeatureExtractor:
    ...
    def create_mel_filterbank(self) -> NDArray[np.float32]:
        ...
        def mel_to_hz(mel: float | NDArray[np.float64]) -> float | NDArray[np.float64]:
        ...
        for idx in range(1, self.n_mels + 1):
            left = bin_points[idx - 1]
            center = bin_points[idx]
            right = bin_points[idx + 1]

            for jdx in range(left, center):
                denom = max(center - left, 1)
                filterbank[idx - 1, jdx] = (jdx - left) / denom
            for jdx in range(center, right):
                denom = max(right - center, 1)
                filterbank[idx - 1, jdx] = (right - jdx) / denom
